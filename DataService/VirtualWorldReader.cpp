...
void VirtualWorldReader::sphereFound(QStringList fields) {
    ...
    if (fields.size() != 16 && fields.size() != 21) { //en cas de TRANSPARENT, inclou 20 (kt + índex de refracció)
    ...
    if (QString::compare("LAMBERTIAN", fields[15], Qt::CaseInsensitive) == 0) {
        ...
        if (fields.size() != 21) { //en cas de TRANSPARENT, inclou 20 (kt + índex de refracció)
